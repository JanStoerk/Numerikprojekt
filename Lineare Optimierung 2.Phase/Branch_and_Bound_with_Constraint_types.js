class BranchAndBound {
    constructor(objectiveCoefficients, constraintsCoefficients, constraintsBounds, constraintTypes) {
        ...
        this.constraintTypes = constraintTypes; // Typen der Nebenbedingungen ('≤', '≥', '<', '>')
    ...
    computeInitialBounds() {
        ...
        return bounds.map(bound => [bound[0], isFinite(bound[1]) ? bound[1] : Math.floor(10 * this.objectiveCoefficients[bounds.length - 1])]); //false bedingung ist einfach 10*der Coeffizient als arbiträre Obergrenze falls kein Endlicher Wert gefunden wird
    }
    ...
    satisfiesConstraints(solution) {
        return this.constraintsCoefficients.every((coeffs, index) => {
            const sum = coeffs.reduce((sum, coeff, varIndex) =>     //die Summe aller Koeffizienten wird erstellt 
                varIndex < solution.length ? sum + coeff * solution[varIndex] : sum, 0);

            switch (this.constraintTypes[index]) {
                case '≤':
                    return sum <= this.constraintsBounds[index];
                case '≥':
                    return sum >= this.constraintsBounds[index];
                case '<':
                    return sum < this.constraintsBounds[index];
...
const constraintTypes = ['≤', '≥', '<']; // Beispiel für gemischte Nebenbedingungen

const maxIterations = 70;

const bbSolver = new BranchAndBound(objectiveCoefficients, constraintsCoefficients, constraintsBounds, constraintTypes);
