...
function extractVariablesAndCoefficients(objectiveFunction, constraints) {
    ...
    // Extrahiere Variablen und Koeffizienten aus den Nebenbedingungen
    constraints.forEach((constraint, index) => {
        const [lhs, rhs] = constraint.split('<=');
        const constraintMap = extractFromExpression(lhs);

        // Speichere den Grenzwert der Ungleichung in der Map
        constraintMap.set('rhs', parseFloat(rhs.trim()));

        console.log(`Constraint ${index + 1} Variables and Coefficients:`);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function getInputs() {
        ...
        extractVariablesAndCoefficients(funktion, nebenbedingungen);
    }
